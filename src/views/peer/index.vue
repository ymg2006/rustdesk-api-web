<template>
  <div>
    <el-card class="list-query query-card"
             shadow="hover">
      ...
      <el-form inline class="filter-form">
        ...
        <el-form-item :label="T('Hostname')">
          <el-input v-model="listQuery.hostname"
                    clearable />
        </el-form-item>
        <el-form-item :label="T('LastOnlineTime')">
          <el-select v-model="listQuery.time_ago"
                     clearable>
            <el-option v-for="item in timeFilters"
                       :key="item.value"
                       :label="item.text"
                       :value="item.value"
        ...
        <el-form-item class="filter-form-ex">
          ...
          <el-button type="danger"
                     @click="toBatchDelete">{{ T('BatchDelete') }}</el-button>
          <el-button type="primary"
                     @click="toBatchAddToAB">{{ T('BatchAdd') }}</el-button>
        </el-form-item>
      </el-form>
    </el-card>
      ...
      <div class="table-view">
        ...
        <el-table class="list-table"
          ...
          <el-table-column type="selection"
                           align="center"
                           width="50" />
          <template v-for="c in visibleColumns.filter(cc => cc.visible)"
                    :key="c">
            ...
            <el-table-column v-if="c.name === 'cpu'"
                             prop="cpu"
                             :label="T('Cpu')"
                             align="center"
                             min-width="100"
                             show-overflow-tooltip />
            <el-table-column v-if="c.name === 'hostname'"
                             prop="hostname"
                             :label="T('Hostname')"
                             align="center"
                             min-width="120" />
            <el-table-column v-if="c.name === 'memory'"
                             prop="memory"
                             :label="T('Memory')"
                             align="center"
                             min-width="120" />
            <el-table-column v-if="c.name === 'os'"
                             prop="os"
                             :label="T('Os')"
                             align="center"
                             min-width="120"
                             show-overflow-tooltip />
            <el-table-column v-if="c.name === 'last_online_time'"
                             prop="last_online_time"
                             :label="T('LastOnlineTime')"
                             align="center"
                             min-width="120">
              <template #default="{ row }">
                <div class="last_oline_time">
                  <span> {{ row.last_online_time ? timeAgo(row.last_online_time * 1000) : '-' }}</span> <span
                        class="dot"
                        :class="{ red: timeDis(row.last_online_time) >= 60, green: timeDis(row.last_online_time) < 60 }"></span>
                </div>
              </template>
            </el-table-column>
            <el-table-column v-if="c.name === 'last_online_ip'"
                             prop="last_online_ip"
                             :label="T('LastOnlineIp')"
                             align="center"
                             min-width="120" />
            <el-table-column v-if="c.name === 'username'"
                             prop="username"
                             :label="T('Username')"
                             align="center"
                             min-width="120" />
            <el-table-column v-if="c.name === 'group_id'"
                             prop="group_id"
                             :label="T('Group')"
                             align="center"
                             min-width="120">
            ...
            <el-table-column v-if="c.name === 'uuid'"
                             prop="uuid"
                             :label="T('Uuid')"
                             align="center"
                             min-width="120"
                             show-overflow-tooltip />
            <el-table-column v-if="c.name === 'version'"
                             prop="version"
                             :label="T('Version')"
                             align="center"
                             min-width="80" />
            <el-table-column v-if="c.name === 'alias'"
                             prop="alias"
                             :label="T('Alias')"
                             align="center"
                             min-width="80" />
            <el-table-column v-if="c.name === 'created_at'"
                             prop="created_at"
                             :label="T('CreatedAt')"
                             align="center"
                             min-width="150" />
            <el-table-column v-if="c.name === 'updated_at'"
                             prop="updated_at"
                             :label="T('UpdatedAt')"
                             align="center"
                             min-width="150" />
          ...
          <el-table-column :label="T('Actions')"
            ...
            <template #default="{ row }">
              <el-button type="success"
                         ...
                         size="small">{{ T('Link') }}</el-button>
              <el-button v-if="appStore.setting.appConfig.web_client"
                         type="success"
                         @click="toWebClientLink(row)"
                         size="small"> {{ T('WebClient') }}
              </el-button>
              <el-button type="primary"
                         @click="toAddressBook(row)"
                         size="small">{{ T('AddToAddressBook') }}</el-button>
              <el-button @click="toEdit(row)"
      ...
      <el-form class="dialog-form"
        ...
        <el-form-item :label="T('Group')"
          ...
          <el-select v-model="formData.group_id">
            <el-option v-for="item in groupListRes.list"
                       :key="item.id"
                       :label="item.name"
        ...
        <el-form-item>
          ...
          <el-button @click="submit"
                     type="primary">{{ T('Submit') }}</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>

    <el-dialog v-model="ABFormVisible"
               width="800"
               :title="T('Create')"
               destroy-on-close
               append-to-body>
      <createABForm :peer="clickRow"
                    @success="ABFormVisible = false"
                    @cancel="ABFormVisible = false"></createABForm>
    </el-dialog>

    <el-dialog v-model="batchABFormVisible"
               width="800"
               :title="T('Create')"
               append-to-body>
      <el-form class="dialog-form"
        ...
        <el-form-item :label="T('Owner')"
                      prop="user_id"
                      required>
          <el-select v-model="batchABFormData.user_id"
                     @change="changeUserForBatchCreateAB">
        ...
        <el-form-item :label="T('AddressBookName')"
                      required
                      prop="collection_id">
          <el-select v-model="batchABFormData.collection_id"
                     clearable>
            <el-option :value="0"
                       :label="T('MyAddressBook')"></el-option>
            <el-option v-for="c in collectionListResForBatchCreateAB.list"
                       :key="c.id"
                       :label="c.name"
        ...
        <el-form-item>
          ...
          <el-button @click="submitBatchAddToAB"
                     type="primary">{{ T('Submit') }}</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>

    <!--     <el-dialog v-model="columnSettingVisible" :title="T('ColumnSetting')" append-to-body>
