...
const conf = {
  ...
  server: {
    ...
    proxy: {
      [process.env.VITE_SERVER_API]: {
        ...
        // rewrite: path => path.replace(/^\/api/, '/api'), // For local simulation
        changeOrigin: true,
      },
    },
  },
  build: {
    target: 'es2020',
    minify: 'esbuild', // Whether to minify: boolean | 'terser' | 'esbuild'; esbuild is used by default
      ...
      output: {
        manualChunks (id) {
          if (id.includes('node_modules')) {
            ...
            // Bundle everything under src together to avoid many small files
            return 'gwen'
          }
        },
